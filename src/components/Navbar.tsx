  ...
  return <header className={`sticky top-0 z-50 w-full transition-all duration-300 ${isScrolled ? "bg-white/90 backdrop-blur-sm shadow-sm py-3" : "bg-transparent py-4"}`}>
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center">
          <a href="tel:+919392206641" className="mr-6 text-gray-700 hover:text-brand-blue flex items-center">
            <span className="font-medium">+91 9392206641</span>
          </a>
          <Button variant="gradient" className="rounded-full" onClick={handleConsultation}>
      ...
      {isMenuOpen && <div className="md:hidden bg-white">
          <div className="container mx-auto px-4 py-4 space-y-4">
            {navLinks.map(link => <a key={link.name} href={link.href} className="block py-2 text-gray-700 hover:text-brand-blue" onClick={() => setIsMenuOpen(false)}>
                {link.name}
              </a>)}
            <div className="pt-4 border-t border-gray-100">
              <a href="tel:+919392206641" className="block py-2 text-gray-700 hover:text-brand-blue">
                +91 9392206641
              </a>
              <Button variant="gradient" className="w-full mt-4 rounded-full" onClick={() => {
            handleConsultation();
            setIsMenuOpen(false);
          }}>
                Get Free Consultation
